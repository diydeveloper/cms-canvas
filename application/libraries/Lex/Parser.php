...
class Lex_Parser
{
	...
	public function parse($text, $data = array(), $callback = false, $allow_php = false)
	{
		...
		$text = $this->extract_noparse($text); // CMS Canvas fix to extract no parse from $data
	...
	protected function setup_regex()
	{
		...
		$this->variable_tag_regex = '/\{\{\s*('.$this->variable_regex.')(\s*=\s*["\'].*?["\'])?\s*\}\}/m'; // CMS Canvas fix to support {{ variable='whatever' }}
