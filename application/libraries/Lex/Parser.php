...
class Lex_Parser
{
    ...
    protected $scopeGlue = '.';
    protected $tagRegex = '';
    // CMS Canvas enable cumulative no parse
    // protected $cumulativeNoparse = false;
    protected $cumulativeNoparse = true;
    ...
    public function parseCallbackTags($text, $data, $callback)
    {
        ...
        while (preg_match($regex, $text, $match, PREG_OFFSET_CAPTURE)) {
            ...
            // CMS Canvas pass data to callback function
            ...
            // CMS Canvas fix to extract noparse from callback return content
            $replacement = $this->extractNoparse($replacement); 
    ...
    public function parseConditionals($text, $data, $callback)
    {
        ...
        foreach ($matches as $match) {
            ...
            // CMS Canvas fix for variable conditionals with variable data that contain callbacks
            $tempCondition = $condition;
            $condition = preg_replace_callback('/\b('.$this->variableRegex.')\b/', array($this, 'processConditionVar'), $condition);

            // CMS Canvas fix for variable conditionals with data that contain callbacks
            // if ($callback) {
            if ($callback && $tempCondition == $condition) {
    ...
    protected function setupRegex()
    {
        ...
        // CMS Canvas added variable {{ variable='whatever' }}
        // $this->variableTagRegex = '/\{\{\s*('.$this->variableRegex.')\s*\}\}/m';
        $this->variableTagRegex = '/\{\{\s*('.$this->variableRegex.')(\s*=\s*["\'].*?["\'])?\s*\}\}/m';
    ...
    protected function parsePhp($text)
    {
        ...
        if ($result === false) {
            ...
            // CMS Canvas adjustment to not use namespaces
            /* throw new LexParsingException($output.str_replace(array('?>', '<?php '), '', $text)); */
            throw new LexParsingException($output.str_replace(array('?>', '<?php '), '', $text));
        }
